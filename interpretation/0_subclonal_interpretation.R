...
for(i in 1:nrow(combs)){
  ...
  mobster_table = tryCatch(
    readRDS(file.path(main_path, "validation_subclonal_new/tables_interpreted", paste0("mobster_univariate_", spn, "_", simulation_id, ".rds"))),
    error = function(e) {
      message("Table not found: ", simulation_id,
              " (", e$message, ")")
      return(NULL)
  ...
  if (is.null(mobster_table)) {
    mobster_table = tool_table %>% 
      select(mutation_id) %>% 
      mutate(mobster_cluster = 'Clonal')
  ...
  table_driver <- final_table %>% 
    filter(!is.na(driver_label_process)) %>% 
    filter(cluster_id_tool %in% drivers_process) %>% 
