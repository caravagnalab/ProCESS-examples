...
color_tool <- c('pyclonevi' = 'sienna2', 
                'viber' = 'mediumpurple4')
...
# mutations_class <- readRDS('/orfeo/cephfs/scratch/area/lvaleriani/races/ProCESS-examples/interpretation/performance_mutations_class.rds')
...
#   )
# 
# saveRDS(object = confusion_per_group_class_mutations, file = '/orfeo/cephfs/scratch/area/lvaleriani/races/ProCESS-examples/interpretation/confusion_mutations.rds')
...
plt_cluster <- confusion_per_group_class %>%
...
plt_mutations <- confusion_per_group_class_mutations %>%
  pivot_longer(cols = c(Precision, Recall, Specificity, F1, Accuracy)) %>%
  ggplot() +
  geom_boxplot(aes(x = class, y = value, col = score_type, fill = score_type), alpha = .6) +
  scale_color_manual('Score', values = color_score) +
