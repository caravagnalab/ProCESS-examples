...
for(i in 1:nrow(combs)){
  ...
  if(univariate==F){
    ...
    # table_process = readRDS(file.path(save_path, "/tables", paste0('table_process_univariate', "_", spn, "_", simulation_id, ".rds")))
    # table_process = table_process %>% mutate(sample_id=paste0(spn, "_", sample_id))
    }
  ...
  if(univariate == F){
    saveRDS(table_to_save, file.path("/orfeo/cephfs/scratch/cdslab/shared/SCOUT/validation_subclonal_SPN02/", paste0(tool, "_", spn, "_", simulation_id, ".rds")))
    # saveRDS(table_to_save, file.path(save_path, "tables_interpreted", paste0(tool, "_", spn, "_", simulation_id, ".rds")))
  }else{
    saveRDS(table_to_save, file.path("/orfeo/cephfs/scratch/cdslab/shared/SCOUT/validation_subclonal_SPN02/", paste0(tool, "_univariate_", spn, "_", simulation_id, ".rds")))
    # saveRDS(table_to_save, file.path(save_path, "/tables_interpreted", paste0(tool, "_", spn, "_", simulation_id, ".rds")))
