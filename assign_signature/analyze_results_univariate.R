...
for (sign_type in c('SBS', 'ID')){
  ...
  for (s in samples){
    ...
    color_palette_process['Tail'] = 'gray70'
    color_palette_process['Other'] = 'salmon3'
...
for (i in 1:length(samples)){
  ...
  for (type in c('raw', 'int')){
    if (type == 'raw'){
      ...
      p_tool_raw = muts_tool_raw %>%
        ...
        theme_bw() +
        ylab('') + 
        scale_fill_manual('Tool clusters Raw', values = color_palette_tool)+
        theme_bw() +
        ggtitle('Raw') +
        guides(color = guide_legend(override.aes = list(size = 3, alpha = 1) ) )
      ...
      p_tool_int = muts_tool_int %>%
        filter(.data[[s1]] != 0) %>% 
        ggplot(aes(x =.data[[s1]], fill=cluster_id_tool_interpreted), alpha=0.2, size = .1)+
        geom_histogram( alpha=0.6, binwidth = 0.01, position = "identity")+
        xlim(-0.01,1.01)+
        xlab(paste0('VAF ', s1)) +
        theme_bw() +
        ylab('') + 
        scale_fill_manual('Tool clusters Int', values = color_palette_tool)+
        theme_bw() +
        ggtitle('Interpreted') +
        guides(color = guide_legend(override.aes = list(size = 3, alpha = 1) ) ) 
      
      p_tool_int = p_tool_int + ggrepel::geom_label_repel(
        data = muts_tool_int %>% filter(is_driver_tool == TRUE) %>% filter(.data[[s1]] != 0),
        aes(
          x = .data[[s1]],
          y = 10,
          label = driver_label,
  ...
  cluster_plots_samples[[s1]] <- p_process + p_tool_raw + p_tool_int
}
...
for (i in 1:length(samples)){
  s = samples[[i]]
  s1 = sub("^(.+)_\\1_", "\\1_", s)
  pfinal[[s1]] <- wrap_plots(cluster_plots_samples[[s]],plot_sign_all[[s1]][['SBS']],plot_sign_all[[s1]][['ID']], nrow = 3, ncol = 1, guides = 'collect')
}
pf <- wrap_plots(pfinal, nrow = length(samples))
ggsave(plot = pf, filename = paste0('/orfeo/cephfs/scratch/area/lvaleriani/races/ProCESS-examples/assign_signature/assign_plot_univariate/', spn, '/',spn,'_',cov, 'x_', pur, 'p_', tool,'_',signature_tool,'.png'), 
       dpi = 400, width = 9.5, height = 4.5*(length(samples)+2))
