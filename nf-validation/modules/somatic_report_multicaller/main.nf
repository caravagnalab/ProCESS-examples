process GENERATE_SOMATIC_REPORT_SINGLE_COMBINATION {
    ...
    output:
    tuple val(spn), val(coverage), val(purity), path("**/*.rds"),	emit:metrics_somatic_rds
    tuple val(spn), val(coverage), val(purity), path("**/*.png"),	emit:metrics_somatic_report
    
    //publishDir "${params.outdir}/somatic/report/", mode: 'copy'
    publishDir "${params.outdir}/somatic/${coverage}x_${purity}p/", mode: 'copy'
