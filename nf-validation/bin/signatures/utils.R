...
align_callers <- function(tumourevo_signature_res,tool,spn){
  if (tool =="SigProfiler"){
    df <- tumourevo_signature_res[["SigProfiler_COSMIC_exposure"]]
    if (is.null(df)) next
    ...
    # Assign proper rownames consistently
    n_samples <- nrow(mat)
    sample_ids <- get_sample_names(spn)
    rownames(mat) <- sample_ids
    ...
    df <- tumourevo_signature_res$BASCULE_refined_fit$nmf[[context_classes]]$exposure
    df <- df %>% 
...
prepare_sankey_data_sbs <- function(ground_truth_list, bascule_aligned, sigprof_aligned) {
  gt_long <- reshape_exposures_long(ground_truth_list)
