...
for(i in 1:nrow(combs)){
  ...
  plot_tool = plot_scatter_tool(final_table, color_palette_tool, sample_names, type ='interpreted', vertical = vertical)
  # plot_tool
  
  plot_tool_raw = plot_scatter_tool(final_table, color_palette_tool, sample_names, type ='original', vertical = vertical)
  # plot_tool_raw
  
  plot_tool_interpreted_driver = plot_scatter_tool(final_table, color_palette_tool, sample_names, type ='interpreted_driver', vertical = vertical)
  ...
  scatter_process = plot_scatter_process(final_table, sample_names, color_palette_process, driver=T, vertical = vertical)
  # scatter_process
...
if(interpreted_driver==T){
  ...
  plot_name = 'multivariates_new_branches/process'
...
for(spn in spns){
  ...
  for(i in 1:nrow(combs)){
    ...
    # Get interpreted table
    final_table = readRDS(file.path(save_path, "tables_interpreted_new_branches", paste0(tool, "_", spn, "_", simulation_id, ".rds"))) # process table in folder tables/
    ...
    color_palette_process["Subclonal"] = "#cccccc"
    # sticks_process = final_table %>%
    #   dplyr::rename(gene=driver_label_process, is_driver=is_driver_process, cluster_id=cluster_id_process) %>%
    #   mutate(cluster_id=ifelse(cluster_id=='Subclonal', 'Neutral', cluster_id))%>%
    #   slice_sample(prop = 0.05)
    # 
    # plot_sticks_process = my_plot_forest_cartoon(forest = sample_forest,
    #                                              sticks_process = sticks_process,
    #                                              color_map = color_palette_process)
    # plot_sticks_process
