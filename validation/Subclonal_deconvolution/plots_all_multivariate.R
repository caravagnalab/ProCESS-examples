...
for(i in 1:nrow(combs)){
  ...
  if(spn=='SPN03' & tool != 'process_viber'){
    final_table = final_table %>% mutate(mutation_id = if_else(
  ...
  plot_tool_raw = plot_scatter_tool(final_table, color_palette_tool, sample_names, type ='original')
  plot_tool_raw

  # Scatterplot process
  color_palette_process = hue_pal()(length(unique(final_table$cluster_id_process))) %>% 
  ...
  scatter_process = plot_scatter_process(final_table, sample_names, color_palette_process, driver=T)
  scatter_process
