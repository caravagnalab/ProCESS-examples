...
for(i in 1:nrow(combs)){
  ...
  # Save the total numver of sample to select clonal clusters (i.e. ccf > 0.95 in n_samples clusters)
  ...
  table_process = table_process %>% 
    # Non va messo qui questo filter? rimuove le altre??
    group_by(cluster_id_process) %>% 
    mutate(
      is_clonal_process = if_else(
        first(cluster_id_process) %in% c$cluster_id_process,
  ...
  # ---------- Find private clusters due to drivers ---------- #
  ...
  private_clonal_clusters = uniques %>%
    ...
    summarise(n = sum(ccf_tool > 0.1, na.rm = TRUE), .groups = "drop") %>%
    filter(n == 1) %>%
    pull(cluster_id_tool)
  ...
  a_filt = a %>%
    ...
    inner_join(
      ccf_tool_drivers  %>% 
        ...
        # transmute(sample_id, driver_ccf = ccf_tool),
      by = "sample_id"
    ) %>%
