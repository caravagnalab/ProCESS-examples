...
cna_data <- lapply(get_process_cna('SPN05'),readRDS)

info_spn <- metadata %>% filter(SPN_ID=='SPN05')

# sample dynamics
setwd('/orfeo/scratch/cdslab/shared/SCOUT/SPN05/process/')
simulation <- ProCESS::recover_simulation('SPN05')
simulation_info <- simulation$get_samples_info()
color_map_clones <- c('Clone 1' = '#4f5d75', 'Clone 2' = '#a0c1b9', 
                      'Clone 3' = '#f4d35e', 'Clone 4' = 'lightsalmon2',
                      'Clone 5' = 'plum4',
                      'Wild-type' = 'white', 
                      `NA` = 'white')
...
plot_sampling <- patchwork::wrap_plots(plot_sample, nrow = 1, ncol = length(samples_name), guides = 'collect') & theme(legend.position = 'none')
...
p = plot_sampling / muller +
  #plot_layout(heights = c(.9, 2)) & 
  theme(text = element_text(size = 12))
p

base <- '/orfeo/cephfs/scratch/area/lvaleriani/races/spn_plot/SPN05'
ggsave(file.path(base, "sampling.pdf"), plot = p, width = 7, height = 5, units = "in", device = cairo_pdf)
ggsave(file.path(base, "sampling.png"), plot = p, width = 7, height = 5, units = "in", dpi = 400)
