...
for (i in 1:nrow(combs)){
  ...
  plt_process <- df_process %>% 
    ...
    geom_line(data = ~ filter(.x, !is.na(value)), aes(x = name, y = value, col = cluster, group = cluster), linewidth = .6, show.legend = F)  +
    geom_text(data = ~ filter(.x, contains_driver & name == 'score_all'), aes(x = name, y = value+0.03, col = cluster, group = cluster, label = driver_label_process), show.legend = F) + 
    ylab('Score') +
    xlab('')+ 
    scale_color_manual('Cluster ProCESS', 
  ...
  plt_tool <- df_tool %>%
    ...
    ylab('Score') +
    xlab('')+
    scale_color_manual('Cluster Tool',
  ...
  plt <- plt_process + plt_tool + wrap_plots(plt_score)+ plot_layout(design = 'ABC')
  ggsave(plot =  plt, 
         filename = '/orfeo/cephfs/scratch/area/lvaleriani/races/ProCESS-examples/figures/edf_figures/spn06/plt_score.pdf',
         height = 3, width = 13)
}