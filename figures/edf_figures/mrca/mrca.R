...
sbs_colors[['SBS11']] = 'steelblue4'
sbs_colors[['SBS25']] = 'violetred'
...
for (sp in spns){
  ...
  for (s in samples){
    ...
    } else if (s %in% c('SPN07_1.3','SPN07_1.1','SPN07_1.2', 'SPN07_2.1', 'SPN07_2.2')){
    ...
    if (nrow(filt_gt)>0){
      ...
      if (sp == 'SPN04'){
        ...
        filt_gt = filt_gt %>% 
          filter(causes %in% names(sbs_colors)) %>% 
          mutate(causes = ifelse(causes =='SBS11', causes, 'Other'))
    ...
    if (sp == 'SPN06'){
      ...
      plots_bar[[sp]] <-  df_spn %>%
        filter(sample_id %in% c("SPN07_SPN07_1.1", "SPN07_SPN07_2.1")) %>% 
        group_by(sample_id, cluster_id_process) %>% 
        summarise(n = n()) %>% 
        filter(cluster_id_process == 'Clonal') %>% 
        mutate(sample_id = sub("^([^_]+)_\\1", "\\1", sample_id)) %>% 
        mutate(sample_id = case_when(
          grepl("SPN07_1.1", sample_id) ~ "Pre",
          grepl("SPN07_2.1", sample_id) ~ "Post"
        )) %>% 
        mutate(sample_id = factor(sample_id, levels = c('Pre', 'Post'))) %>% 
        ggplot()+
        geom_col(aes(x = sample_id, y = n, fill = cluster_id_process), position = "dodge") + 
...
forest = load_sample_forest(get_sample_forest(spn = 'SPN07'))
sub_07 <- forest$get_subforest_for(c("SPN07_1.1", "SPN07_2.1"))
f_spn07 <- plot_forest(sub_07, color_map = c("Clone 1"='gainsboro',
                                             "Clone 2"='gainsboro',
                                             "Clone 3"='gainsboro',
                                             "Clone 4"='gainsboro',
                                             "Clone 5"='gainsboro'))  %>%
  annotate_forest(sub_07)
...
sig_07 <- df_simultated_signatures %>% 
  filter(sample %in% c('SPN07_1.1', 'SPN07_2.1')) %>% 
  filter(signature %in% names(sbs_colors)) %>% 
  filter(coverage == 100, purity == 0.9) %>% 
  mutate(sample = case_when(
    grepl("SPN07_1.1", sample) ~ "Pre",
    grepl("SPN07_2.1", sample) ~ "Post"
  )) %>% 
  ggplot() + 
  geom_col(aes(y = sample, x = exposure, fill =signature )) +
  my_ggplot_theme() + 
...
spn07 <- free(f_spn07 +  theme(legend.position = 'none')) + 
  plots_sample$SPN07_1.1 + 
  plots_sample$SPN07_2.1 + 
  plots_bar$SPN07 + 
  plots_sign$SPN07_2.1 + 
  sig_07 + 
  plot_layout(guides = 'collect', nrow = 1, design = 'AAA#BBDEE\nAAA#CCDFF') 


all <- wrap_plots(spn04, spn06, spn07, nrow = 3, guides = 'collect')
